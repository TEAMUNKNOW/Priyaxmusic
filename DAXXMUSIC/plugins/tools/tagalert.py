...
async def locks_dfunc(_, message):
   ...
   if len(message.command) != 2:
      return await lol.edit("ᴇxᴘᴇᴄᴛᴇᴅ ᴏɴ ᴏʀ ᴏғғ ᴇx :- `/tagalert ON`")
   ...
   if parameter == "on" or parameter=="ON":
     ...
     if not isittrue:
          ...
          return await lol.edit(f"ᴛᴀɢ ᴀʟᴇʀᴛ ᴇɴᴀʙʟᴇᴅ.\nᴡʜᴇɴ sᴏᴍᴇᴏɴᴇ ᴛᴀɢs ʏᴏᴜ ᴀs @{uname} ʏᴏᴜ ᴡɪʟʟ ʙᴇ ɴᴏᴛɪғɪᴇᴅ")
     else:
          return await lol.edit("ᴛᴀɢ ᴀʟᴇʀᴛs ᴀʟʀᴇᴀᴅʏ ᴇɴᴀʙʟᴇᴅ ғᴏʀ ʏᴏᴜ")
   if parameter == "off" or parameter=="OFF":
     if not message.from_user:
       return
     if not message.from_user.username:
       return await lol.edit("ᴏɴʟʏ ᴜsᴇʀs ᴡɪᴛʜ ᴜsᴇʀɴᴀᴍᴇs ᴀʀᴇ ᴇʟɪɢɪʙʟᴇ ғᴏʀ ᴛᴀɢ ᴀʟᴇʀᴛ sᴇʀᴠɪᴄᴇ")
     uname = message.from_user.username
     uname = uname.lower()
     isittrue = tagdb.find_one({f"teg": uname})
     if isittrue:
          tagdb.delete_one({f"teg": uname})
          return await lol.edit("ᴛᴀɢ ᴀʟᴇʀᴛs ʀᴇᴍᴏᴠᴇᴅ")
     else:
          return await lol.edit("ᴛᴀɢ ᴀʟᴇʀᴛs ᴀʟʀᴇᴀᴅʏ ᴅɪsᴀʙʟᴇᴅ ғᴏʀ ʏᴏᴜ") 
   else:
...
async def mentioned_alert(client, message):
    try:
        ...
        final_tagged_msg = f"**ʏᴏᴜ ʜᴀᴠᴇ ʙᴇᴇɴ** [Tagged]({tagged_msg_link}) **ɪɴ** {chat_name} **ʙʏ** {user_}"
